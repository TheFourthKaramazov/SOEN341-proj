import json
import logging
import os
import secrets
import string
from contextlib import asynccontextmanager
from io import BytesIO
import subprocess
from typing import Set, Optional
from urllib import request

from PIL import Image
from fastapi import (
    FastAPI,
    Depends,
    Header,
    WebSocket,
    WebSocketDisconnect,
    HTTPException,
    UploadFile,
    File,
    Form,
)
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import FileResponse, JSONResponse
from fastapi.staticfiles import StaticFiles
from sqlalchemy.orm import Session

from app.backend.database import SessionLocal, init_db, get_db
from app.backend.models import (
    User,
    DirectMessage,
    Channel,
    ChannelMessage,
    UserChannel,
    Image as ImageModel,
    Video as VideoModel,
)
from app.backend.schemas import (
    ChannelResponse,
    UserCreate,
    DirectMessageCreate,
    ChannelCreate,
    ChannelMessageCreate,
)
from typing import List


@asynccontextmanager
async def lifespan(app: FastAPI):
    init_db()  # ensures DB is initialized
    yield


app = FastAPI(lifespan=lifespan, max_upload_size=100 * 1024 * 1024)

# Directories for uploaded files
image_dir = "./app/backend/media/images"
os.makedirs(image_dir, exist_ok=True)
video_dir = "./app/backend/media/videos"
os.makedirs(video_dir, exist_ok=True)

# Mount static
app.mount("/media/images", StaticFiles(directory=image_dir), name="images")
app.mount("/media/videos", StaticFiles(directory=video_dir), name="videos")

# Global connections
active_connections = {}
channel_connections = {}
global_channel_connections: Set[WebSocket] = set()

# CORS
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Database
def get_db():
    db = SessionLocal()
    try:
        yield db
    finally:
        db.close()


# WebSockets

@app.websocket("/ws/channel/{channel_id}")
async def websocket_endpoint(websocket: WebSocket, channel_id: int, db: Session = Depends(get_db)):
    await websocket.accept()
    if channel_id not in channel_connections:
        channel_connections[channel_id] = []
    channel_connections[channel_id].append(websocket)
    print(f"✅ Client connected to channel {channel_id}")

    try:
        while True:
            data = await websocket.receive_json()
            sender_id = data.get("sender_id")
            message_text = data.get("text")
            if not sender_id or not message_text:
                continue

            print(f"Message in channel {channel_id}: {data}")

            msg = ChannelMessage(channel_id=channel_id, sender_id=sender_id, text=message_text)
            db.add(msg)
            db.commit()
            db.refresh(msg)

            response_data = {
                "id": msg.id,
                "channel_id": channel_id,
                "sender_id": sender_id,
                "text": message_text,
            }
            for ws_conn in channel_connections[channel_id]:
                await ws_conn.send_json(response_data)

    except WebSocketDisconnect:
        print(f"Client disconnected from channel {channel_id}")
        channel_connections[channel_id].remove(websocket)


@app.websocket("/realtime/direct/{user_id}")
async def websocket_direct(websocket: WebSocket, user_id: int, db: Session = Depends(get_db)):
    await websocket.accept()
    if user_id not in active_connections:
        active_connections[user_id] = set()
    active_connections[user_id].add(websocket)

    try:
        while True:
            text_data = await websocket.receive_text()
            try:
                message = json.loads(text_data)
                msg_type = message.get("type", "direct")
                sender_id = user_id

                if msg_type == "direct":
                    receiver_id = message.get("receiver_id")
                    msg_text = message.get("content")
                    new_dm = store_direct_message(db, sender_id, receiver_id, msg_text)
                    response_data = {
                        "type": "direct",
                        "id": new_dm["id"],
                        "sender_id": sender_id,
                        "receiver_id": receiver_id,
                        "content": msg_text,
                    }
                    if receiver_id in active_connections:
                        for ws_conn in active_connections[receiver_id]:
                            await ws_conn.send_json(response_data)
                    for ws_conn in active_connections[sender_id]:
                        await ws_conn.send_json(response_data)

                elif msg_type == "channel":
                    channel_id = message.get("receiver_id") or message.get("channel_id")
                    msg_text = message.get("content") or message.get("text")
                    ch_msg = ChannelMessage(channel_id=channel_id, sender_id=sender_id, text=msg_text)
                    db.add(ch_msg)
                    db.commit()
                    db.refresh(ch_msg)
                    response_data = {
                        "type": "channel",
                        "id": ch_msg.id,
                        "channel_id": channel_id,
                        "sender_id": sender_id,
                        "text": msg_text,
                    }
                    for uid, ws_set in active_connections.items():
                        for ws_conn in ws_set:
                            await ws_conn.send_json(response_data)

            except json.JSONDecodeError as e:
                print(f"[ERROR] Invalid JSON: {text_data}, {e}")
                continue

    except WebSocketDisconnect:
        print(f"[INFO] WebSocket disconnected: user_id={user_id}")
        active_connections[user_id].remove(websocket)
        if not active_connections[user_id]:
            del active_connections[user_id]


@app.websocket("/realtime/channel/{channel_id}/{user_id}")
async def websocket_channel(websocket: WebSocket, channel_id: int, user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter_by(id=user_id).first()
    channel = db.query(Channel).filter_by(id=channel_id).first()
    await websocket.accept()

    if channel_id not in active_connections:
        active_connections[channel_id] = set()
    active_connections[channel_id].add(websocket)

    try:
        while True:
            data = await websocket.receive_json()
            sender_id = data.get("sender_id")
            message_text = data.get("text")
            if sender_id != user_id:
                await websocket.send_json({"error": "Unauthorized sender."})
                continue

            new_msg = ChannelMessage(channel_id=channel_id, sender_id=sender_id, text=message_text)
            db.add(new_msg)
            db.commit()
            db.refresh(new_msg)

            response_data = {
                "channel_id": channel_id,
                "sender_id": sender_id,
                "text": message_text,
            }
            for conn in active_connections[channel_id]:
                await conn.send_json(response_data)

    except WebSocketDisconnect:
        active_connections[channel_id].remove(websocket)
        if not active_connections[channel_id]:
            del active_connections[channel_id]


@app.websocket("/realtime/global/channels")
async def websocket_global_channels(websocket: WebSocket, db: Session = Depends(get_db)):
    await websocket.accept()
    global_channel_connections.add(websocket)
    try:
        while True:
            data = await websocket.receive_text()
    except WebSocketDisconnect:
        global_channel_connections.remove(websocket)
    except Exception as e:
        print(f"WebSocket error: {e}")
        global_channel_connections.remove(websocket)


async def broadcast_channel_update(message: str):
    for conn in global_channel_connections.copy():
        try:
            await conn.send_text(message)
        except Exception as e:
            print(f"Failed to send message: {e}")
            global_channel_connections.remove(conn)


# Routes

@app.post("/login")
def login(user: UserCreate, db: Session = Depends(get_db)):
    existing_user = db.query(User).filter(User.username.ilike(user.username)).first()
    if not existing_user:
        raise HTTPException(status_code=404, detail="User does not exist.")
    if existing_user.password_hash != user.password:
        raise HTTPException(status_code=401, detail="Incorrect password")
    return {"id": existing_user.id, "username": existing_user.username, "is_admin": existing_user.is_admin}


@app.get("/users/")
def get_users(db: Session = Depends(get_db)):
    users = db.query(User).all()
    return [{"id": u.id, "name": u.username} for u in users]


def store_direct_message(db: Session, sender_id: int, receiver_id: int, text: str):
    sender = db.query(User).filter(User.id == sender_id).first()
    receiver = db.query(User).filter(User.id == receiver_id).first()
    if not sender or not receiver:
        raise HTTPException(status_code=404, detail="Sender or receiver not found")

    dm = DirectMessage(sender_id=sender_id, receiver_id=receiver_id, text=text)
    db.add(dm)
    db.commit()
    db.refresh(dm)
    return {
        "id": dm.id,
        "sender_id": sender_id,
        "receiver_id": receiver_id,
        "text": dm.text,
        "timestamp": dm.timestamp
    }


def store_channel_message(db: Session, sender_id: int, channel_id: int, text: str):
    sender = db.query(User).filter(User.id == sender_id).first()
    channel = db.query(Channel).filter(Channel.id == channel_id).first()
    if not sender or not channel:
        raise HTTPException(status_code=404, detail="Sender or channel not found")

    ch_msg = ChannelMessage(sender_id=sender_id, channel_id=channel_id, text=text)
    db.add(ch_msg)
    db.commit()
    db.refresh(ch_msg)
    return {
        "id": ch_msg.id,
        "sender_id": sender_id,
        "channel_id": channel_id,
        "text": ch_msg.text,
        "timestamp": ch_msg.timestamp
    }


@app.post("/messages/")
def create_direct_message(dm: DirectMessageCreate, db: Session = Depends(get_db)):
    new_msg = store_direct_message(db, dm.sender_id, dm.receiver_id, dm.text)
    return {"message": new_msg}


@app.post("/channel-messages/")
def create_channel_message(msg: ChannelMessageCreate, db: Session = Depends(get_db)):
    new_msg = store_channel_message(db, msg.sender_id, msg.channel_id, msg.text)
    return {
        "id": new_msg["id"],
        "channel_id": new_msg["channel_id"],
        "text": new_msg["text"]
    }


@app.get("/messages/{user1_id}/{user2_id}")
def get_messages(user1_id: int, user2_id: int, db: Session = Depends(get_db)):
    msgs = (
        db.query(DirectMessage)
        .filter(
            ((DirectMessage.sender_id == user1_id) & (DirectMessage.receiver_id == user2_id))
            | ((DirectMessage.sender_id == user2_id) & (DirectMessage.receiver_id == user1_id))
        )
        .order_by(DirectMessage.timestamp.asc())
        .all()
    )
    user_map = {
        u.id: u.username
        for u in db.query(User).filter(User.id.in_([user1_id, user2_id])).all()
    }
    return [
        {
            "id": m.id,
            "sender_id": m.sender_id,
            "receiver_id": m.receiver_id,
            "sender_name": user_map.get(m.sender_id, f"User {m.sender_id}"),
            "receiver_name": user_map.get(m.receiver_id, f"User {m.receiver_id}"),
            "text": m.text,
            "timestamp": m.timestamp
        }
        for m in msgs
    ]


@app.get("/channel-messages/{channel_id}")
def get_channel_messages(channel_id: int, db: Session = Depends(get_db)):
    messages = (
        db.query(ChannelMessage)
        .filter(ChannelMessage.channel_id == channel_id)
        .order_by(ChannelMessage.timestamp.asc())
        .all()
    )
    return [{"id": m.id, "sender_id": m.sender_id, "text": m.text} for m in messages]


@app.post("/channels/")
async def create_channel(channel: ChannelCreate, db: Session = Depends(get_db), user_id: int = Header(None)):
    logging.debug(f"Request to create channel. user_id={user_id}")
    try:
        chan_name = channel.name.strip()
        if not chan_name:
            raise HTTPException(status_code=400, detail="Channel name cannot be blank")

        existing = db.query(Channel).filter(Channel.name.ilike(chan_name)).first()
        if existing:
            raise HTTPException(status_code=400, detail="Channel name already exists")

        current_user = db.query(User).filter(User.id == user_id).first()
        if not current_user:
            logging.error(f"User not found: {user_id}")
            raise HTTPException(status_code=404, detail="User not found")
        if not current_user.is_admin:
            logging.error(f"User {user_id} not an admin")
            raise HTTPException(status_code=403, detail="Only admins can create channels")

        new_channel = Channel(name=chan_name, is_public=True)
        db.add(new_channel)
        db.commit()
        db.refresh(new_channel)
        logging.debug(f"Channel created: {new_channel}")

        await broadcast_channel_update(json.dumps({
            "event": "channel_created",
            "channel": {
                "id": new_channel.id,
                "name": new_channel.name,
                "is_public": new_channel.is_public
            }
        }))
        return new_channel
    except HTTPException as e:
        raise e
    except Exception as e:
        logging.error(f"Error creating channel: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")


@app.delete("/delete_channel/{channel_id}")
async def delete_channel(channel_id: int, user_id: int = Header(...), db: Session = Depends(get_db)):
    current_user = db.query(User).filter(User.id == user_id).first()
    if not current_user:
        raise HTTPException(status_code=404, detail="User not found")
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Only admins can delete channels")

    to_delete = db.query(Channel).filter(Channel.id == channel_id).first()
    if not to_delete:
        raise HTTPException(status_code=404, detail="Channel not found")

    db.delete(to_delete)
    db.commit()
    await broadcast_channel_update(json.dumps({
        "event": "channel_deleted",
        "channel_id": channel_id
    }))
    return {"message": "Channel deleted successfully"}


@app.post("/join_channel/{channel_id}")
def join_channel(channel_id: int, user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    channel = db.query(Channel).filter(Channel.id == channel_id).first()
    if not user or not channel:
        raise HTTPException(status_code=404, detail="User or channel not found")

    membership = db.query(UserChannel).filter_by(user_id=user.id, channel_id=channel_id).first()
    if membership:
        return {"message": "Already a member"}

    new_member = UserChannel(user_id=user.id, channel_id=channel_id)
    db.add(new_member)
    db.commit()
    return {"message": "Successfully joined the channel"}


@app.get("/channels/", response_model=List[ChannelResponse])
def get_channels(user_id: int = Header(None), db: Session = Depends(get_db)):
    public_channels = db.query(Channel).filter(Channel.is_public == True).all()
    if user_id:
        user_private = (
            db.query(Channel)
            .join(UserChannel, Channel.id == UserChannel.channel_id)
            .filter(UserChannel.user_id == user_id)
            .all()
        )
        available = public_channels + user_private
    else:
        available = public_channels

    if not available:
        raise HTTPException(status_code=404, detail="No channels found")
    return available


@app.delete("/channel-messages/{message_id}")
async def delete_channel_message(message_id: int, db: Session = Depends(get_db), user_id: int = Header(None)):
    current_user = db.query(User).filter(User.id == user_id).first()
    if not current_user:
        raise HTTPException(status_code=404, detail="User not found")
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Only admins can delete messages")

    msg = db.query(ChannelMessage).filter(ChannelMessage.id == message_id).first()
    if not msg:
        raise HTTPException(status_code=404, detail="Message not found")

    ch_id = msg.channel_id
    db.delete(msg)
    db.commit()

    await notify_message_deleted(ch_id, message_id)
    return {"message": "Message deleted successfully"}


async def notify_message_deleted(channel_id: int, message_id: int):
    if channel_id in active_connections:
        for ws_conn in active_connections[channel_id]:
            try:
                await ws_conn.send_json({
                    "action": "message_deleted",
                    "type": "channel",
                    "channel_id": channel_id,
                    "message_id": message_id
                })
                print("Notified a client about channel message deletion")
            except Exception as e:
                print(f"Failed to notify client: {e}")
    else:
        print(f"No active connections found for channel {channel_id}")


@app.delete("/direct-messages/{message_id}")
async def delete_direct_message(message_id: int, db: Session = Depends(get_db), user_id: Optional[str] = Header(None)):
    user_id = int(user_id)
    current_user = db.query(User).filter(User.id == user_id).first()
    if not current_user:
        raise HTTPException(status_code=404, detail="User not found")
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Only admins can delete messages")

    dm = db.query(DirectMessage).filter(DirectMessage.id == message_id).first()
    if not dm:
        raise HTTPException(status_code=404, detail="Message not found")

    receiver_id = dm.receiver_id
    sender_id = dm.sender_id
    db.delete(dm)
    db.commit()

    for uid in [sender_id, receiver_id]:
        if uid in active_connections:
            for ws_conn in active_connections[uid]:
                await ws_conn.send_json({
                    "action": "message_deleted",
                    "type": "direct",
                    "message_id": message_id,
                    "sender_id": sender_id,
                    "receiver_id": receiver_id
                })
    return {"message": "Direct message deleted successfully"}


# Helpers

@app.post("/upload")
async def upload_image(file: UploadFile = File(...), uploader_id: int = Form(...)):
    file_id = f"{generate_media_id()}.png"
    file_path = os.path.join(image_dir, file_id)

    contents = await file.read()
    with open("temp_upload", "wb") as tmp:
        tmp.write(contents)

    with Image.open(BytesIO(contents)) as img:
        width, height = img.size
    with Image.open("temp_upload") as img:
        img.save(file_path, format="PNG")

    db = SessionLocal()
    db_image = ImageModel(filename=file_id, uploader_id=uploader_id, width=width, height=height)
    db.add(db_image)
    db.commit()
    db.refresh(db_image)
    os.remove("temp_upload")

    return {"filename": file_id}


@app.post("/upload-video")
async def upload_video(file: UploadFile = File(...), uploader_id: int = Form(...), db: Session = Depends(get_db)):
    max_size = 50 * 1024 * 1024
    file.file.seek(0, 2)
    file_size = file.file.tell()
    file.file.seek(0)
    if file_size > max_size:
        raise HTTPException(status_code=413, detail="File too large (max 50MB)")

    allowed_types = ["video/mp4", "video/webm", "video/quicktime"]
    if file.content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="Unsupported video format")

    ext = os.path.splitext(file.filename)[1]
    file_id = f"{generate_media_id()}{ext}"
    file_path = os.path.join(video_dir, file_id)

    try:
        contents = await file.read()
        with open(file_path, "wb") as f:
            f.write(contents)
        w, h = get_video_dimensions(file_path)

        db_video = VideoModel(filename=file_id, uploader_id=uploader_id, width=w, height=h)
        db.add(db_video)
        db.commit()

        return {"filename": file_id}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error uploading video: {e}")
    finally:
        db.close()


def generate_media_id(length=6):
    alphabet = string.ascii_letters + string.digits
    return ''.join(secrets.choice(alphabet) for _ in range(length))


def get_video_dimensions(file_path: str) -> tuple[int, int]:
    try:
        cmd = [
            'ffprobe', '-v', 'error',
            '-select_streams', 'v:0',
            '-show_entries', 'stream=width,height',
            '-of', 'csv=p=0',
            file_path
        ]
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, text=True)
        return tuple(map(int, result.stdout.strip().split(',')))
    except (subprocess.CalledProcessError, FileNotFoundError):
        pass
    return (0, 0)


@app.get("/homepage-images/{user_id}")
def get_homepage_images(user_id: int, db: Session = Depends(get_db)):
    media_items = []
    subs = [uc.channel_id for uc in db.query(UserChannel).filter(UserChannel.user_id == user_id).all()]

    dm_image_msgs = db.query(DirectMessage).filter(
        DirectMessage.text.startswith("[IMAGE:"),
        (DirectMessage.sender_id == user_id) | (DirectMessage.receiver_id == user_id)
    ).order_by(DirectMessage.timestamp.desc()).limit(10).all()

    for msg in dm_image_msgs:
        if "[IMAGE:" in msg.text:
            filename = msg.text.split("[IMAGE:")[1].split("]")[0]
            img = db.query(ImageModel).filter_by(filename=filename).first()
            if img:
                media_items.append({
                    "type": "image",
                    "filename": filename,
                    "width": img.width,
                    "height": img.height,
                    "timestamp": msg.timestamp.isoformat(),
                    "direction": "incoming" if msg.sender_id != user_id else "outgoing",
                    "other_user_id": msg.receiver_id if msg.sender_id == user_id else msg.sender_id,
                    "source": "dm"
                })

    dm_video_msgs = db.query(DirectMessage).filter(
        DirectMessage.text.startswith("[VIDEO:"),
        (DirectMessage.sender_id == user_id) | (DirectMessage.receiver_id == user_id)
    ).order_by(DirectMessage.timestamp.desc()).limit(10).all()

    for msg in dm_video_msgs:
        if "[VIDEO:" in msg.text:
            filename = msg.text.split("[VIDEO:")[1].split("]")[0]
            vid = db.query(VideoModel).filter_by(filename=filename).first()
            if vid:
                media_items.append({
                    "type": "video",
                    "filename": filename,
                    "width": vid.width,
                    "height": vid.height,
                    "timestamp": msg.timestamp.isoformat(),
                    "direction": "incoming" if msg.sender_id != user_id else "outgoing",
                    "other_user_id": msg.receiver_id if msg.sender_id == user_id else msg.sender_id,
                    "source": "dm"
                })

    if subs:
        channel_msgs = db.query(ChannelMessage).filter(
            ChannelMessage.channel_id.in_(subs),
            ChannelMessage.text.startswith("[")
        ).order_by(ChannelMessage.timestamp.desc()).limit(20).all()

        for m in channel_msgs:
            if "[IMAGE:" in m.text:
                filename = m.text.split("[IMAGE:")[1].split("]")[0]
                img = db.query(ImageModel).filter_by(filename=filename).first()
                chan = db.query(Channel).filter_by(id=m.channel_id).first()
                if img:
                    media_items.append({
                        "type": "image",
                        "filename": filename,
                        "width": img.width,
                        "height": img.height,
                        "timestamp": m.timestamp.isoformat(),
                        "direction": "channel",
                        "other_user_id": m.sender_id,
                        "source": "channel",
                        "channel_id": m.channel_id,
                        "channel_name": chan.name if chan else "Unknown",
                    })
            elif "[VIDEO:" in m.text:
                filename = m.text.split("[VIDEO:")[1].split("]")[0]
                vid = db.query(VideoModel).filter_by(filename=filename).first()
                chan = db.query(Channel).filter_by(id=m.channel_id).first()
                if vid:
                    media_items.append({
                        "type": "video",
                        "filename": filename,
                        "width": vid.width,
                        "height": vid.height,
                        "timestamp": m.timestamp.isoformat(),
                        "direction": "channel",
                        "other_user_id": m.sender_id,
                        "source": "channel",
                        "channel_id": m.channel_id,
                        "channel_name": chan.name if chan else "Unknown",
                    })

    media_items.sort(key=lambda x: x["timestamp"], reverse=True)
    return media_items


@app.get("/test-random-images")
def get_test_images(db: Session = Depends(get_db)):
    print("/test-random-images route hit")
    imgs = db.query(ImageModel).order_by(ImageModel.id.desc()).limit(10).all()
    return [
        {
            "filename": i.filename,
            "width": i.width,
            "height": i.height,
            "uploader_id": i.uploader_id,
        }
        for i in imgs
    ]


@app.post("/subscribe/{user_id}/{channel_id}")
def subscribe_to_channel(user_id: int, channel_id: int, db: Session = Depends(get_db)):
    exists = db.query(UserChannel).filter_by(user_id=user_id, channel_id=channel_id).first()
    if not exists:
        db.add(UserChannel(user_id=user_id, channel_id=channel_id))
        db.commit()
    return {"subscribed": True}


@app.delete("/unsubscribe/{user_id}/{channel_id}")
def unsubscribe_from_channel(user_id: int, channel_id: int, db: Session = Depends(get_db)):
    sub = db.query(UserChannel).filter_by(user_id=user_id, channel_id=channel_id).first()
    if sub:
        db.delete(sub)
        db.commit()
    return {"subscribed": False}


@app.get("/is-subscribed/{user_id}/{channel_id}")
def is_subscribed(user_id: int, channel_id: int, db: Session = Depends(get_db)):
    exists = db.query(UserChannel).filter_by(user_id=user_id, channel_id=channel_id).first()
    return {"subscribed": bool(exists)}


logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger(__name__)
